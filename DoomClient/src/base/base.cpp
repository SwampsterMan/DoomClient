...
void Base::Init()
{
	...
	case MinecraftVersion::UNKNOWN:
		...
		MessageBoxA(nullptr, "Unknown Minecraft version detected", "DoomClient", MB_OK | MB_ICONERROR);
		Logger::Shutdown();
		MH_Uninitialize();
		Java::Shutdown();
	...
	if (!HotSpot::Init())
	{
		...
		MessageBoxA(nullptr, "Failed to initialize HotSpot", "DoomClient", MB_OK | MB_ICONERROR);
		Logger::Shutdown();
		MH_Uninitialize();
		Java::Shutdown();
		FreeLibraryAndExitThread(Main::hModule, 0);
...
void Base::renderLoop() // Runs every frame
{
	...
	if (settings::Hud_Watermark)
	{
		...
		const char* watermark = "DoomClient";
