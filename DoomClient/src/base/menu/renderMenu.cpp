...
void Menu::RenderMenu()
{
	...
	ImGui::Begin("DoomClient", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
	{
		ImVec2 titleSize = Menu::fontBold->CalcTextSizeA(24, FLT_MAX, 0.0f, ("DoomClient" + std::string(Base::m_version)).c_str());
		...
		{
			Menu::BoldText("DoomClient", FontSize::SIZE_24);
			ImGui::SameLine();
				...
				if (selectedModule == 0) // Local Configs
				{
					...
					ImGui::BeginChild("###ConfigLits", ImVec2(0, 436.f));
					{
						...
						for (int i = 0; i < configFiles.size(); ++i)
						{
							...
							if (deleted)
							{
								...
								{
									NotificationManager::Send("DoomClient :: Config", "Config \"%s\" has been removed.", configFiles[i].c_str());
								}
								else
								{
									NotificationManager::Send("DoomClient :: Config", "Config \"%s\" could not be removed.", configFiles[i].c_str());
								}
					...
					if (Menu::Button(("Load \"" + selectedConfigName + "\"").c_str(), ImVec2(ImGui::GetWindowSize().x - openButtonSize.x - refreshButtonSize.x - 56.f, 30.f)))
					{
						if (configFiles.size() > 0)
						{
							...
							{
								NotificationManager::Send("DoomClient :: Config", "Config \"%s\" has been loaded.", selectedConfigName.c_str());
								Menu::ResetSetupFlags();
							}
							else
							{
								NotificationManager::Send("DoomClient :: Config", "Config \"%s\" could not be loaded.", selectedConfigName.c_str());
							}
						}
					}
					...
					{
						configFiles = configmanager::GetConfigList();
						NotificationManager::Send("DoomClient :: Config", "Config list has been refreshed.");
					}
					...
					if (Menu::Button("Save", ImVec2(saveButtonSize.x, 30.f)))
					{
						...
						if (result != -1)
						{
							...
							NotificationManager::Send("DoomClient :: Config", "Config \"%s\" has been saved.", saveConfigName);
						}
						else
						{
							NotificationManager::Send("DoomClient :: Config", "Config \"%s\" could not be saved.", saveConfigName);
						}
					}
				}
			...
			else if (selectedCategory == -2) // Settings
			{
				...
				else if (selectedModule == 1) // Friends
				{
					...
					ImGui::BeginChild("###FriendsLits", ImVec2(0, 476.f));
					{
						...
						for (int i = 0; i < friends.size(); ++i)
						{
							...
							if (deleted)
							{
								...
								{
									NotificationManager::Send("DoomClient :: Friends", "Friend \"%s\" has been removed.", friends[i].c_str());
								}
								else
								{
									NotificationManager::Send("DoomClient :: Friends", "Friend \"%s\" could not be removed.", friends[i].c_str());
								}
							}
						}
					}
					...
					if (Menu::Button("Add", ImVec2(addFriendButtonSize.x, 30.f)))
					{
						...
						{
							NotificationManager::Send("DoomClient :: Friends", "Friend \"%s\" has been added.", newFriendName);
						}
						else
						{
							NotificationManager::Send("DoomClient :: Friends", "Friend \"%s\" could not be added.", newFriendName);
						}
					}
				}
