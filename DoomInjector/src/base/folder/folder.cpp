...
{
    return GetFusionFolder() + "\\DoomClient" + GetVersionStringDll() + ".dll";
}
...
std::string FolderManager::GetVersionStringDll()
{
    ...
    {
        if (entry.path().extension() == ".dll" && entry.path().filename().string().starts_with("DoomClient"))
        {
...
{
    return FolderManager::GetDocumentsPath("DoomClient");
}
