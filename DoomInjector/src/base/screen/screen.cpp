...
static void UpdateInjectorThread()
{
	...
	if (BaseUtils::UpdateInjector()) {
		...
		std::string path = FolderManager::GetCurrentDir() + "DoomInjector" + BaseUtils::new_injetor_version + ".exe";
...
bool Screen::Render()
{
	...
	ImGui::Begin("Doom Injector", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoNavFocus | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoNav);
	ImGui::PopStyleVar();
	...
	if (ImGui::Button(text.c_str()))
	{
		if (minecraft_running)
		{
			...
			{
				MessageBoxA(NULL, "Failed to inject!", "Doom Injector", MB_OK | MB_ICONERROR);
			}
		}
		else
		{
			MessageBoxA(NULL, "Minecraft is not running!", "Doom Injector", MB_OK | MB_ICONERROR);
		}
	}
	ImGui::PopStyleVar();
	...
	{
		ShellExecute(0, L"open", L"https://discord.gg/tXan2VtwtX", 0, 0, SW_SHOWNORMAL);
	}
	ImGui::SameLine();
	if (ImGui::Button("Github"))
	{
		ShellExecute(0, L"open", L"https://github.com/SwampsterMan/DoomClient", 0, 0, SW_SHOWNORMAL);
	}

	std::string version = " DoomClient v" + dllVersion + " | DoomInjector v" + injectorVersion;
